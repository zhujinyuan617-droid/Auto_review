...
- **管线换代(2026-06-10,feature/pipeline-regen 分支)**:代码与测试已完成——默认链收敛为每篇 4 次 AI 调用
  ...
  词表改由注册表纯脚本派生(AI 归一退役),authorship+机构注册表,抽取并行化。
  **更正(2026-06-10 晨):全库回填九步已跑完**(状态见 ISSUES I18);归一已换**批量判同 bulk**
  (提案并行/落账串行,实测 2 小时 → 3.2 分钟,验收数字见 docs/superpowers/archive/2026-06-10-speed-sp-design.md §6)。
...
`docs/superpowers/archive/2026-06-09-element-index-design.md` 与两份 2026-06-10 文档(数据框架、管线换代)。
