...
3. `scripts/use/verify_angle.py` — DeepSeek 读原文判 真/假/有条件矛盾(怀疑式)。
4. 用户对着证据定夺 → `scripts/use/draft_section.py` 出稿。

**一次实证(2026-06,记录教训)**:角度"vug 对效率的矛盾(S08 vs S28)"——propose 标为 contradiction;
verify 第一次判 false 是因为**取句截断漏了 S08 的关键结论句**(修:取句优先含方向词/结论段)。修后:
DeepSeek 说"两篇量的是**不同指标**(sweep vs displacement),不算真矛盾";我手动补充"**润湿性也不同**(水湿 vs 油湿)"。
**合判:这是软/假矛盾,角度降级。** 两条教训:(1) 整链最弱环是"取原文",取不全 AI 必判错;
(2) 两个独立读者(我 + DeepSeek)对一对,比信任任何单个都强 —— 验证拦住了围绕假矛盾写综述。
