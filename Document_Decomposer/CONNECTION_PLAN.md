...
3. `scripts/use/verify_angle.py` — **对抗式全文复核**:喂全文(不按词检索)→ 控方/辩方各逐字举证 →
   脚本逐字核真引文 → 裁判只认核真引文判 真/假/有条件;**默认纯 pro**(实测 flash 在难对子自信判错)、
   信心非 high 交人裁。(详细见 `scripts/README.md`;设计/bug/选型证据见 ISSUES I14。)
4. 用户对着证据定夺 → `scripts/use/draft_section.py` 出稿。

**实证 + 元发现(2026-06)**:抽样复核 7/15 条 contradicts 边 → **0 干净真矛盾、2 假、5 有条件**
(调和变量集中在体系条件:基质/尺度/温度/相态)。即 contradicts 边**系统性过报**,逐条复核才可信;
而"同一效应随体系条件反号、缺受控统一"反倒可能是更值钱的综述角度。详见 ISSUES I15。
