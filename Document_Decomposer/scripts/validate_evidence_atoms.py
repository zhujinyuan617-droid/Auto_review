...
from docdecomp.evidence_synthesis import (
    ...
    write_evidence_atoms_report,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate generated evidence atoms.")
    ...
    parser.add_argument("--report", default=str(ROOT / "reports" / "evidence_atoms_quality.csv"))
...
def main() -> int:
    ...
    paper_dirs = [library_dir / paper_id for paper_id in args.paper_id] if args.paper_id else sorted(
        path for path in library_dir.iterdir() if path.is_dir() and (path / args.atoms_name).exists()
    )
    ...
    for paper_dir in paper_dirs:
        ...
        rows.append(validate_evidence_atoms(package, reading))

    report_path = Path(args.report)
    write_evidence_atoms_report(report_path, rows)
    print(f"Wrote {report_path}")
    for row in rows:
        print(
            f"{row['paper_id']}: {row['status']}; atoms={row['atom_count']}; "
            f"unknown_rb={row['unknown_reading_block_count']}; bad_source={row['bad_source_ref_count']}; "
            f"page_mismatch={row['page_mismatch_count']}; quote_not_found={row['quote_not_found_count']}; "
